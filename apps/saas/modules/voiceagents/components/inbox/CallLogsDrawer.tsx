...
import type { GatewayCallEvent } from "@repo/api/modules/voiceagents/procedures/calls";
...
/**
 * "AI logs" side panel for a call: everything the AI sent/received to make
 * decisions — LLM calls (ai.turn), tool invocations (tool.*), CRM HTTP
 * traffic (http.request) and the engine's flow/lifecycle breadcrumbs — as a
 * newest-first, filterable log list (CloseBot parity, our design system).
 *
 * Unknown/legacy event types always render as plain info rows: the drawer
 * must never crash on an old call's events.
 */
...
export function CallLogsDrawer({
	...
	events: GatewayCallEvent[] | undefined;
	isLoading: boolean;
	onClose: () => void;
}) {
			...
			<div className="gap-2 p-3 min-h-0 flex flex-1 flex-col overflow-y-auto">
				{isLoading ? (
					...
					<p className="py-8 text-sm text-center text-muted-foreground">
						No logs recorded for this call.
					</p>
				) : (
...
function LogRow({ event }: { event: GatewayCallEvent }) {
...
function AiRow({ event }: { event: GatewayCallEvent }) {
...
function ToolRow({ event }: { event: GatewayCallEvent }) {
	const [isOpen, setIsOpen] = useState(false);
	const p = event.payload;
...
function HttpRow({ event }: { event: GatewayCallEvent }) {
	const [isOpen, setIsOpen] = useState(false);
	const p = event.payload;
...
function InfoRow({ event }: { event: GatewayCallEvent }) {
...
function infoSummary(event: GatewayCallEvent): string {
